...
public class BigBoss3 extends BigBoss{

    public static float vx;
    public static float vy;

    public BigBoss3(Hero hero){
        super(hero,battlePanel.bigBoss3);
        this.hero = hero;
        x = 50F;
        y = -260F;
        power = 1000;
    ...
    /*public void move() {
        ...
        //tamaIntCount 控制发射子弹的间隔，在间隔之间只移动，不发射子弹
        if (onlyMoveCount > 0)
            onlyMoveCount--;
        if (onlyMoveCount <= 0) {
            battlePanel.addList(new FirstEnemyBullet(x + WIDTH / 2.0F, y
                    + HEIGHT, hero, battlePanel.SecondEnemyBulletImage));
            battlePanel.addList(new SecondEnemyBullet(x + WIDTH / 2.0F,
                    y + HEIGHT, hero, battlePanel.BeamEnemyBulletImage));
            onlyMoveCount = 50;
        }
        counter++;
    }
*/
    ...
    protected boolean checkHit(Role chara) {
        ...
        if (x - 21F > chara.x - WIDTH
                && x + 21F < chara.x + chara.WIDTH
